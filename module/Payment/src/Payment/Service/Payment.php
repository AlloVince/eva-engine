...
class Payment
{
     ...
     */
    public function __construct($adapter = 'PaypalEc', $direction = false, $options = array())
    {
    ...
    public function setAdapter($adapter, $direction = false, $options = array())
    {
        ...
        if ($adapter[0] != '\\') {
            $adapter = '\Payment\Service\Adapter\\' . ucfirst($adapter);
        }
        ...
        if (!$this->adapter[$direction] instanceof Adapter\AbstractAdapter) {
            throw new Exception\InvalidArgumentException(
                'Adapter ' . $adapter . ' does not extend Core\Tree\Adapter\AbstractAdapter'
            );
        }

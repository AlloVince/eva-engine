...
CREATE TRIGGER `conversations_update` AFTER UPDATE ON `eva_message_conversations`
 FOR EACH ROW UPDATE `eva_message_indexes` SET `messageCount` = (SELECT count(id) FROM `eva_message_conversations` WHERE `eva_message_conversations`.`user_id` = NEW.user_id AND `eva_message_conversations`.`author_id` = NEW.author_id AND `eva_message_conversations`.`readFlag` = 0) WHERE `eva_message_indexes`.`user_id` = NEW.user_id AND `eva_message_indexes`.`author_id` = NEW.author_id AND NEW.`readFlag` = 1 AND NEW.`readFlag` != OLD.`readFlag`
//
DELIMITER ;
...
CREATE TRIGGER `conversations_delete` AFTER DELETE ON `eva_message_conversations`
 ...
 UPDATE `eva_message_indexes` SET `messageCount` = (SELECT count(id) FROM `eva_message_conversations` WHERE `eva_message_conversations`.`user_id` = OLD.user_id AND `eva_message_conversations`.`author_id` = OLD.author_id AND `eva_message_conversations`.`readFlag` = 0) WHERE `eva_message_indexes`.`user_id` = OLD.user_id AND `eva_message_indexes`.`author_id` = OLD.author_id;
 END
//
DELIMITER ;
